$(function() {
        ...
        if (ids != cachedIds) {
          ...
          req.onreadystatechange = function (){
            if (this.readyState == 4 && this.status == 200) {
              tagScore(this.responseText, newDivs);

              // Re-Sort, append, cache, update html
              newDivs = $(newDivs).sort(SORT_TYPE);
              storeCache(queryUrl, ids, newDivs);
  ...
  $("#sort-select").on("click", "li #sort-new", function(event) {
    ...
    SORT_TYPE  = (IS_USING_WATCHED) ? dateNewSort : dateSort; 
    divs = divs.sort(SORT_TYPE);
    $.each($(".feed-list"), function(index, page) { 
      if (index != 0) { $(page).html('') };
    });
  ...
  function storeCache(queryUrl, ids, newDivs) {
  ...
  //
  function tagScore(json, newDivs) {
